...
def main():
    ...
    for cutoff_date, train_df, test_df in get_rolling_window_splits(df, start_date):
        ...
        model, best_params, best_value = tune_lgbm_with_optuna(X_train, y_train, train_df['date'], n_trials=500)
        final_model_params = best_params
        quantile_models = train_lgbm_quantile_models(X_train, y_train, best_params)
        lower_95, preds, upper_95 = [quantile_models[q].predict(X_test) for q in (0.025, 0.5, 0.975)]
        train_lower_95, train_preds, train_upper_95 = [quantile_models[q].predict(X_train) for q in (0.025, 0.5, 0.975)]
    ...
    save_predictions(predictions_df, OUTPUT_DIR / 'lgbm_test_predictions.csv')
    save_wis_by_state_date(predictions_df, OUTPUT_DIR / 'lgbm_test_wis_by_state_date.csv')

    if final_model_params is not None:
        X_all, y_all, _, _, _ = prepare_common_features(df, df, drop_cols=('date',))
        final_model = tune_lgbm_with_optuna(X_all, y_all, df['date'], n_trials=500)[0]
        save_model(final_model, MODEL_DIR / 'lgbm_model.joblib')
